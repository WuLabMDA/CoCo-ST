...
#' res <- CoCoST(t(data1), W1, t(data2), W2)
#' }
#'
#' @export
CoCoST <- function(data1, W1, data2, W2, para = 0.2, Dim = 2) {
  ...
  if (!requireNamespace("Matrix", quietly = TRUE))
    stop("Package 'Matrix' is required. Please install it.")
  if (!requireNamespace("RSpectra", quietly = TRUE))
    stop("Package 'RSpectra' is required. Please install it.")
  ...
  D1inv_sqrt <- Matrix::Diagonal(x = 1 / sqrt(rowSums(W1)))
  L1 <- D1 - W1
  normL1 <- D1inv_sqrt %*% L1 %*% D1inv_sqrt

  D2 <- Matrix::Diagonal(x = rowSums(W2))
  D2inv_sqrt <- Matrix::Diagonal(x = 1 / sqrt(rowSums(W2)))
  L2 <- D2 - W2
  normL2 <- D2inv_sqrt %*% L2 %*% D2inv_sqrt

  # Adjust Laplacians
  L11 <- Matrix::Diagonal(nrow(D1)) - (0.2) * normL1
  L22 <- Matrix::Diagonal(nrow(D2)) - (0.2) * normL2
  ...
  # Extract contrastive components
